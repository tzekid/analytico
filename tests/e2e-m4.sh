...
test "$("$binary" doctor "$source_dir")" = \
    "ok metadata=v10 events=v7 sites=2 goals=1 funnels=1 stored_events=3 key=ok"
...
for backup in "$backup_one" "$backup_two"; do
    ...
    test "$(jq -r '.metadata_schema' "$backup/manifest.json")" = 10
...
test "$("$binary" doctor "$legacy_dir")" = \
    "ok metadata=v10 events=v7 sites=0 goals=0 funnels=0 stored_events=1000000 key=ok"
