...
test "$("$binary" doctor "$source_dir")" = \
    "ok metadata=v3 events=v3 sites=2 goals=1 funnels=1 stored_events=3 key=ok"
...
for backup in "$backup_one" "$backup_two"; do
    ...
    test "$(jq -r '.metadata_schema' "$backup/manifest.json")" = 3
...
test "$("$binary" doctor "$source_dir")" = \
    "ok metadata=v3 events=v3 sites=1 goals=1 funnels=1 stored_events=1 key=ok"
