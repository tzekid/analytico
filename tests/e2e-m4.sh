...
test "$("$binary" doctor "$source_dir")" = \
    "ok metadata=v4 events=v4 sites=2 goals=1 funnels=1 stored_events=3 key=ok"
...
test "$("$binary" doctor "$source_dir")" = \
    "ok metadata=v4 events=v4 sites=1 goals=1 funnels=1 stored_events=1 key=ok"
...
test "$("$binary" migrate "$legacy_dir" "$legacy_backup")" = \
    "migrated metadata=v4 events=v4"
test "$("$binary" doctor "$legacy_dir")" = \
    "ok metadata=v4 events=v4 sites=0 goals=0 funnels=0 stored_events=1000000 key=ok"
