...
"$binary" site add "$source_dir" active Active https://active.example >/dev/null
"$binary" site add "$source_dir" retired Retired https://retired.example >/dev/null
...
test "$("$binary" doctor "$source_dir")" = \
    "ok metadata=v2 events=v3 sites=2 goals=1 funnels=1 stored_events=3 key=ok"
...
for backup in "$backup_one" "$backup_two"; do
    ...
    test "$(jq -r '.event_schema' "$backup/manifest.json")" = 3
...
test "$("$binary" doctor "$source_dir")" = \
    "ok metadata=v2 events=v3 sites=1 goals=1 funnels=1 stored_events=1 key=ok"
...
test "$("$binary" migrate "$legacy_dir")" = "migrated metadata=v2 events=v3"
test "$("$binary" doctor "$legacy_dir")" = \
    "ok metadata=v2 events=v3 sites=0 goals=0 funnels=0 stored_events=1000000 key=ok"
...
"$binary" site add "$fault_dir" fault Fault https://fault.example >/dev/null
