...
test "$init_output" = "initialized metadata=v4 events=v6 key=created"
...
test "$("$binary" init "$fixture_dir")" = \
    "initialized metadata=v4 events=v6 key=existing"
...
test "$doctor" = \
    "ok metadata=v4 events=v6 sites=1 goals=3 funnels=1 stored_events=1 key=ok"
...
test "$("$binary" doctor "$fixture_dir")" = \
    "ok metadata=v4 events=v6 sites=0 goals=0 funnels=0 stored_events=0 key=ok"
