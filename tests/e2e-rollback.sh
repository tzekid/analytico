...
"$current" site add "$live" rollback Rollback https://rollback.example >/dev/null
"$current" event add "$live" rollback pageview /before \
    1785456000000000 2026-07-31 203.0.113.1 Chrome Linux desktop >/dev/null
backup="$fixture/pre-upgrade"
"$current" backup "$live" "$backup" >/dev/null
...
test "$("$current" doctor "$live")" = \
    "ok metadata=v1 events=v2 sites=1 goals=0 funnels=0 stored_events=2 key=ok"

rolled_back="$fixture/rolled-back"
"$current" restore "$backup" "$rolled_back" --verify >/dev/null
test "$("$previous" doctor "$rolled_back")" = \
    "ok metadata=v1 events=v2 sites=1 goals=0 funnels=0 stored_events=1"
...
port=$((47000 + ($$ % 1000)))
"$previous" serve "$rolled_back" 127.0.0.1 "$port" \
