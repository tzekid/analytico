...
grep -qi '^Content-Type: text/javascript; charset=utf-8' \
    "$fixture_dir/headers"
grep -qi '^Content-Length: 8071' "$fixture_dir/headers"
...
expect_code 200 -H 'Accept-Encoding: gzip' "$base/tracker.6de111c9.js"
grep -qi '^Cache-Control: public, max-age=31536000, immutable' \
    "$fixture_dir/headers"
grep -qi '^Content-Encoding: gzip' "$fixture_dir/headers"
cmp "$fixture_dir/body" src/http/tracker.6de111c9.min.js.gz
expect_code 200 -H 'Accept-Encoding: br' "$base/tracker.bc506cfe.js"
grep -qi '^Cache-Control: public, max-age=31536000, immutable' \
    "$fixture_dir/headers"
grep -qi '^Content-Encoding: br' "$fixture_dir/headers"
cmp "$fixture_dir/body" public/tracker.js.br
...
test "$("$binary" doctor "$fixture_dir")" = \
    "ok metadata=v8 events=v7 sites=2 goals=0 funnels=0 stored_events=45 key=ok"
...
test "$custom_row" = $'signup\t/welcome\t\tUS\tChrome\tWindows\tdesktop\t\t{\"plan\":\"basic\",\"z\":2}\t1\t2\t\t0\tfalse\t0\tfalse\tfalse\t0\t0\t3\tfalse'
...
test "$("$binary" doctor "$fault_dir")" = \
    "ok metadata=v8 events=v7 sites=1 goals=0 funnels=0 stored_events=0 key=ok"
