...
sqlite3 "$after_drop/meta.db" 'DELETE FROM meta_migrations WHERE version=8;'
...
test "$("$current" migrate "$live")" = "migrated metadata=v8 events=v7"
...
sqlite3 "$overflow/meta.db" \
    'SELECT id FROM goal_definitions ORDER BY id;' \
...
cmp "$fixture/overflow-ids-before-recovery.txt" \
    "$fixture/overflow-ids-after-recovery.txt"
test "$(sqlite3 "$overflow/meta.db" \
    'SELECT count(*) FROM goal_definitions WHERE archived_at_utc_micros IS NULL;')" = 32
...
test "$(sqlite3 "$fresh/meta.db" \
    "SELECT count(*) FROM sqlite_master WHERE type='table' AND name='goal_definitions';")" = 1
test "$(sqlite3 "$fresh/meta.db" \
    "SELECT count(*) FROM sqlite_master WHERE type='table' AND name='goals';")" = 0

printf 'metadata8_migration_e2e=pass predecessor=54f49ed309638da04eb1cd3fd67652832727068a metadata=7-to-8 events=7 rows=preserved saved_views=preserved reports=v1+v2 interruption=partial-copy+after-drop replay=idempotent overflow=34-preserved+analysis-blocked+mutations-recovered rollback=matched-pair\n'
