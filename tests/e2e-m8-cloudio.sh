...
"$binary" auth configure "$data" "$dashboard_origin" >/dev/null
analytico_setup_url=$("$binary" auth bootstrap "$data" --ttl 10m | sed -n '2p')
...
run_browser() {
    ...
    TMPDIR=/tmp \
        ...
        "$analytico_session" "$storage_state" \
        ${extra:+"$extra"}
}
...
start_analytico
{
    printf '{\n\tadmin off\n}\n'
    sed \
        -e "s|^analytics\\.example {|http://localhost:$dashboard_port {|" \
...
TMPDIR=/tmp NODE_PATH="$module_root" \
    ...
    "$dashboard_origin" "$analytico_setup_url" "$analytico_cookie_file"
analytico_session=$(<"$analytico_cookie_file")
...
for _ in {1..100}; do
    status=$(curl --silent --output /dev/null --write-out '%{http_code}' \
        --cookie "analytico_session=$analytico_session" \
        "$dashboard_origin/admin" || true)
    [[ "$status" == 502 ]] && break
    sleep 0.02
done
