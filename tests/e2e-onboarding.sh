...
grep -Fq 'metadata=v6 events=v7' "$fixture/init.txt"
...
jq -e '
    ...
    .metadata_schema == 6 and .event_schema == 7 and
...
printf 'onboarding_e2e=pass metadata=6 events=7 sites=1 events_stored=1 cold_onboarding_rss_growth_kib=%s browser=%s\n' \
